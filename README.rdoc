...
you will still need to pass the --app command when issuing Heroku commands. For example, your config file might look like this:
...
This would then require you use the --app option when issuing a Heroku command. For example:
  ...
  heroku restart --app example-dev
