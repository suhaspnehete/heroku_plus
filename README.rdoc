...
*NOTE*: Account switching will not work properly if all accounts are not suffixed with the .credentials file extension.
...
For each account beyond the first, you’ll need to switch to each Heroku app directory and type the following command:

  git remote add heroku git@official.heroku.com:<your app>.git.

This will allow you to push and deploy your changes back to the Heroku servers and finalizes your setup.
